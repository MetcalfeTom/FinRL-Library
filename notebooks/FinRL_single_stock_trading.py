...
from finrl.trade.backtest import backtest_plot, backtest_stats, baseline_stats
...
data_df = YahooDownloader(
    start_date="2009-01-01", end_date="2021-01-01", ticker_list=["AAPL"]
).fetch_data()
...
trained_ddpg = agent.train_model(
    model=model_ddpg, tb_log_name="ddpg", total_timesteps=30000
)
...
agent = DRLAgent(env=env_train)
TD3_PARAMS = {"batch_size": 128, "buffer_size": 1000000, "learning_rate": 0.0003}
...
trained_sac = agent.train_model(
    model=model_sac, tb_log_name="sac", total_timesteps=30000
)
...
df_account_value, df_actions = DRLAgent.DRL_prediction(
    model=trained_sac, trade_data=trade, test_env=env_trade, test_obs=obs_trade
)
